...
export default function AuthForm({ mode }: { mode: "login" | "signup" }) {
  ...
  const [accountMode, setAccountMode] = useState<"new" | "join">("new");
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (isSignup && step === "details") {
      ...
      if (password !== confirmPassword) { setMessage("Passwords do not match."); setBusy(false); return; }
      if (!termsAccepted) { setMessage("Please accept the terms to continue."); setBusy(false); return; }
  ...
  return <main className="auth-page"><section className="auth-art"><div className="auth-brand"><span className="brand-mark">M</span><strong>Messmate</strong></div><div className="auth-art-copy"><div className="eyebrow">HOUSE 08 / MANAGEMENT</div><h1>Keep the house<br /><em>in balance.</em></h1><p>One clear place for meals, members, money, and the everyday rhythm of your mess.</p><div className="auth-stats"><div><strong>24</strong><span>active members</span></div><div><strong>৳48k</strong><span>collected this month</span></div></div></div></section><section className="auth-panel"><div className="auth-panel-inner"><div className="auth-kicker">{verificationStep ? "CHECK YOUR EMAIL" : isSignup ? "CREATE YOUR WORKSPACE" : "WELCOME BACK"}</div><h2>{verificationStep ? "Verify your email" : isSignup ? "Start your mess" : "Sign in to Messmate"}</h2><p className="auth-subtitle">{verificationStep ? `Enter the 6-digit code sent to ${email}.` : isSignup ? "The first verified account becomes Super Admin automatically." : "Your house is waiting for you."}</p><form onSubmit={submit}>{verificationStep ? <><label>Verification code<input inputMode="numeric" pattern="[0-9]{6}" maxLength={6} value={code} onChange={event => setCode(event.target.value)} placeholder="000000" required /></label><div className="demo-code">Demo email code: <strong>{sentCode}</strong><small>Static GitHub Pages preview only</small></div></> : <>{isSignup && <><div className="account-tabs"><button type="button" className={accountMode === "new" ? "selected" : ""} onClick={() => setAccountMode("new")}>Create new mess</button><button type="button" className={accountMode === "join" ? "selected" : ""} onClick={() => setAccountMode("join")}>Join existing mess</button></div><label>Full name<input value={name} onChange={event => setName(event.target.value)} placeholder="e.g. Farhan Ahmed" required /></label>{accountMode === "new" && <label>Mess name<input placeholder="e.g. House 08" required /></label>}</>}<label>Email address<input type="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="you@example.com" required /></label><label>Password<input type="password" value={password} onChange={event => setPassword(event.target.value)} placeholder="At least 8 characters" minLength={8} required /></label>{isSignup && <><label>Confirm password<input type="password" value={confirmPassword} onChange={event => setConfirmPassword(event.target.value)} placeholder="Repeat your password" minLength={8} required /></label><label className="terms"><input type="checkbox" checked={termsAccepted} onChange={event => setTermsAccepted(event.target.checked)} /> I agree to the Terms of Service and Privacy Policy.</label></>}</>}{message && <div className="auth-message">{message}</div>}<button className="auth-submit" disabled={busy}>{busy ? "Please wait..." : verificationStep ? "Verify & continue →" : isSignup ? "Send verification code →" : "Sign in →"}</button></form>{verificationStep && <button className="resend" onClick={() => pendingUser && sendCode(pendingUser)}>Resend code</button>}<div className="auth-switch">{verificationStep ? "Wrong email?" : isSignup ? "Already have an account?" : "New to Messmate?"} <Link href={verificationStep ? "/signup" : isSignup ? "/login" : "/signup"}>{verificationStep ? "Start again" : isSignup ? "Sign in" : "Create an account"}</Link></div><div className="auth-note">Email verification is a static demo preview. Production delivery needs a backend email provider and server-side sessions.</div></div></section></main>;
}
